...
Attach all four App IDs to that App Group. Enable Family Controls for the app and every extension.
...
2. Select the `FocusApp` project and set the same Apple Developer Team for the app and all three extensions.
3. Keep automatic signing enabled.
4. Confirm each target shows both Family Controls and App Groups under Signing & Capabilities.
...
5. Confirm the block clears after one minute while Focus is not foregrounded.
...
1. In App Store Connect, create the Focus app record using `com.dash1971.focusapp`.
...
- Exercise, reading, cleaning, and custom challenges are honesty-confirmed. Math challenges are automatically verified.
